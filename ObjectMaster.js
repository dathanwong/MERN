...
const idDivideBy3 = pokemon.filter( p => p.id % 3 == 0);
console.log(idDivideBy3);
...
console.log(names);
const id99 = pokemon.filter(p => p.id > 99).map(p => p.name);
console.log(id99);
const onlyPoison = pokemon.filter(p => p.types[0] == "poison" && p.types.length==1).map(p => p.name);
